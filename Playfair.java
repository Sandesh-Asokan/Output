...
class Playfair {
    ...
    public static String prepareText(String s, boolean chgJtoI) {
        s = s.toUpperCase().replaceAll("[^A-Z]", "");
        return chgJtoI ? s.replace("J", "I") : s.replace("Q", "");
    }

    public static void createTbl(String key, boolean chgJtoI) {
        ...
        for (int z = 0; z < charTable.length; z++) {
            for (int y = 0; y <= 4; y++)
                System.out.print(charTable[z][y] + " ");
            System.out.print("\n");
        }
    }
